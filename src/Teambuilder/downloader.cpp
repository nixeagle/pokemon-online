...
Downloader::Downloader(QWidget *parent, QUrl url, QString filePath, QString cusText)
    : QDialog(parent)
{
...
void Downloader::downloadFile()
{
    ...
    QString fileName = QString("%1%2").arg(fileFilePath, fileInfo.fileName());
    qDebug() << fileName;
...
void Downloader::updateProgressRead(qint64 bytesRead, qint64 totalBytes)
{
    ...
    double speed = bytesRead * 1000.0 / downloadTime.elapsed(); // We calculate the download speed.
    QString to;
    if(speed < 1024) {
        to = "Bytes/s";
    } else {
